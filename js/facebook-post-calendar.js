...
function generatePrompt() {
  ...
  const promptText = `Ignora tutte le istruzioni precedenti. Sei un esperto di marketing su Facebook. Hai un tono di voce ${tone}. Hai uno stile di scrittura ${style}. Crea un calendario di post su Facebook per ${months} mesi basato sull’argomento "${topic}" per il tuo pubblico di riferimento ${target}. Devono esserci ${postsPerWeek} post su Facebook programmati ogni settimana del mese. Ogni post su Facebook deve avere un titolo e una descrizione accattivanti. Utilizza emoji uniche nella descrizione. La descrizione deve avere un hook e invogliare i lettori. La tabella deve riportare le date effettive del futuro. Ogni mese deve avere una propria tabella. Le colonne della tabella devono essere: data, descrizione dell'idea del post. Organizza ogni post di Facebook nella tabella in modo che assomigli a un calendario. Non fare riferimento a se stessi. Non spiegare cosa state facendo. Rispondi solo con la tabella.`;
